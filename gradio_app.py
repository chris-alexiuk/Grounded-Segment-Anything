...
def run_grounded_sam(
    ...
    if task_type == "det":
        ...
        if inpaint_mode == "merge":
            ...
            mask = masks[0][0].cpu().numpy()  # convert to numpy array
        ...
        image = pipe(
            prompt=inpaint_prompt, image=image_pil, mask_image=mask_pil
        ).images[0]
...
if __name__ == "__main__":
        ...
        with gr.Row():
            with gr.Column():
                input_image = gr.Image(
                    source="upload", type="pil", value="assets/demo1.jpg"
                )
                task_type = gr.Dropdown(
                    ["det", "seg", "inpainting", "automatic"],
                    value="automatic",
                    label="task_type",
                )
            ...
            with gr.Column():
                gallery = gr.outputs.Image(type="pil",).style(
                    full_width=True, full_height=True
                )
    ...
    block.launch(
        server_name="0.0.0.0", server_port=args.port, debug=args.debug, share=args.share
    )
